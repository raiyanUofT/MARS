...
for s = 1:length(subjects)
    subjectDir = fullfile('./', subjects{s});
...
for i = 1:length(allData)
    ...
    for f = 1:length(uniqueFrames)
        ...
        if numPoints > 1
            [eigVecs, eigValsMatrix] = eig(covMatrix);
...
% Data Saving to .mat
save('allRadarDataWithComprehensiveFeatures.mat', 'processedData', '-v7.3');
...
% Save to CSV
writetable(allFeaturesTable, 'allRadarDataWithComprehensiveFeatures.csv');
